...
function runTutorial(x, y, calloutContent, highlight) {
  ...
  if (document.getElementsByClassName("tutorialCallout").length == 0) {
    ...
    document.getElementsByClassName("tutorialCallout")[0].onclick =
      runTutCallouts;
  } else {
    document
      .getElementsByClassName("tutorialCallout")[0]
      .parentElement.removeChild(
        document.getElementsByClassName("tutorialCallout")[0]
      );
    document
      .getElementsByClassName("tutorialCalloutPrevious")[0]
      .parentElement.removeChild(
        document.getElementsByClassName("tutorialCalloutPrevious")[0]
      );
    document.body.innerHTML += `<div class="tutorialCallout" style="top:0; left:0; border-top-right-radius:0px; display: none;"><a style="z-index: 10005 !important;">${calloutContent}</a></div>`;
    document.body.innerHTML += `<div class="tutorialCalloutPrevious" style="top: 0px;left: 0px;">&#x2190</div>`;
    document.getElementsByClassName("tutorialCallout")[0].onclick =
  ...
  if (highlight) {
    ...
    if (
      document.getElementsByClassName("tutorialCalloutHighlight").length == 0
    ) {
  ...
  var calloutHighlight = document.getElementsByClassName(
    "tutorialCalloutHighlight"
  )[0];
  var calloutPrevious = document.getElementsByClassName(
    "tutorialCalloutPrevious"
  )[0];
  callout.style.display = "unset";
  callout.style.left = `${
    x - document.getElementsByClassName("tutorialCallout")[0].offsetWidth
  }px`;
  callout.style.top = `${y}px`;
  calloutHighlight.style.display = "unset";
  calloutHighlight.style.left = `${
    highlightX -
    document.getElementsByClassName("tutorialCallout")[0].offsetWidth
  }px`;
  calloutHighlight.style.top = `${highlightY}px`;
  calloutPrevious.style.left = `${
    x - document.getElementsByClassName("tutorialCallout")[0].offsetWidth
  }px`;
  calloutPrevious.style.top = `${y}px`;
  document.getElementsByClassName("tutorialCallout")[0].onclick =
    runTutCallouts;
  document.getElementsByClassName("tutorialCalloutPrevious")[0].onclick =
    previousCallout;
}
...
callouts[0] = [
  219 + 42,
  41,
  "This is the schedule button, use it to quickly check your schedule, including room numbers, teachers('emails), class names, and period numbers. Click to continue",
];
callouts[1] = [
  259 + 42,
  41,
  "This is the grades button, use it to quickly check your grades. To make sure it works, please open your school's aeries.net site <br> &nbsp&nbsp&nbsp&nbsp manually before using",
];
callouts[2] = [
  299 + 42,
  41,
  "This is the reset button, use it to soft-reset your customization settings, but be sure to click save to keep the reset options.",
];
callouts[3] = [
  339 + 42,
  41,
  "This is the logo of the extention, which also serves as a link to a quick feedback / bug reports survey. Clicking it will also rerun the &nbsp&nbsp&nbsp&nbsp tutorial.",
];
function runTutCallouts() {
  if (iterations <= callouts.length - 1) {
    ...
    document
      .getElementsByClassName("tutorialOverlay")[0]
      .parentElement.removeChild(
        document.getElementsByClassName("tutorialOverlay")[0]
      );
    document
      .getElementsByClassName("tutorialCalloutHighlight")[0]
      .parentElement.removeChild(
        document.getElementsByClassName("tutorialCalloutHighlight")[0]
      );
...
function pushToFront() {
  var frontElements = [
    ...
    document.getElementById("gradesButton"),
  ...
  chrome.tabs.query({ url: "*://*.aeries.net/student/*" }, function (tabs) {
    if (tabs.length != 0) {
      for (var tab of tabs) {
        ...
        linkToDash =
          "https://" + splitLink[2] + "/" + splitLink[3] + "/Dashboard.aspx";
        linkToClasses =
          "https://" + splitLink[2] + "/" + splitLink[3] + "/Classes.aspx";
        console.log(`created link: ${linkToDash}`);
        console.log(`created link: ${linkToClasses}`);
      }
      chrome.storage.sync.set(
        { dashLink: linkToDash, classLink: linkToClasses },
        function () {
...
function open_feedback() {
  window.open(
    "https://docs.google.com/forms/d/e/1FAIpQLScRFkl3fTB_d0nBePbrHBS7gjHXBgfGjcxeALL8QMX3JEaiRQ/viewform?usp=sf_link",
    "_blank"
  );
...
function createOverview(classes) {
  classElements =
    "<img id=backButton src=../icon/3DLogo.png style=position:absolute;top:5px;right:5px;height:30px>";
  for (var thisclass of classes) {
    console.log(thisclass);
    var classElement = `<div class=elementGroup style=""><div class="gradeSection classLeft" style="color: ${getColor(
      thisclass.grade
    )};">${thisclass.grade}</div><div class="gradeSection classMiddle">${
      thisclass.name
    }</div><div class="gradeSection classRight">${
      thisclass.percent
    }</div></div>`;
  ...
  classElements = classElements.concat(
    `<div class="top-rounded">Quick Check Grades</div>`
  );
  console.log(`allElements: ${classElements}`);
  //applying
  document.getElementsByTagName("body")[0].style =
    "margin-bottom: 25px !important;font-family:montserrat,sans-serif;width:380px;margin:0;margin-top:60px";
...
function update_colors() {
  ...
  document.getElementById(
    "sidebar"
  ).style = `background: linear-gradient(0.25turn, ${sidebar1}, ${sidebar2});`;
  ...
  document.getElementById(
    "loginColorPreview"
  ).style = `background: ${correctColor(
    document.getElementById("loginColor").value
  )} !important;`;
  document.getElementById("bgPreview").style = `background: ${correctColor(
    document.getElementById("solidColor").value
  )} !important;`;
  ...
  chrome.storage.sync.get(
    {
      ...
      useImage: false,
      imgLink2:
        "https://lh3.googleusercontent.com/-24SGkkk23w4/YL6HMZfiIYI/AAAAAAAAygY/43yk0zyDv_MfZXK2ZjxuvBM4Jgieik--wCJEEGAsYHg/s0/2021-06-07.png",
      sColor1: "#2e8eab",
      sColor2: "#113e75",
      remove: true,
      floatBtn: true,
    ...
    function (items) {
      ...
      document.getElementById(
        "leftSidebar"
      ).style = `background: ${items.sColor1};`;
      document.getElementById(
        "rightSidebar"
      ).style = `background: ${items.sColor2};`;
      document.getElementById("district").checked = items.remove;
      document.getElementById(
        "sidebar"
      ).style = `background: linear-gradient(0.25turn, ${items.sColor1}, ${items.sColor2});`;
      document.getElementById("navFloat").checked = items.floatBtn;
      document.getElementById("loginColor").value = items.background;
      document.getElementById(
        "loginColorPreview"
      ).style = `background: ${items.background} !important;`;
      document.getElementById(
        "bgPreview"
      ).style = `background: ${items.bgColor} !important;`;
      document.getElementById("autoLog").checked = items.automaticallyLogin;
...
function emailInputCheck() {
  ...
  if (re.test(document.getElementById("loginEmail").value) == true) {
    document.getElementById("loginEmailCheck").style =
...
function resetSettings() {
  ...
  document.getElementById(
    "sidebar"
  ).style = `background: linear-gradient(0.25turn, ${"#2e8eab"}, ${"#113e75"});`;
...
function classesOverview() {
  //    try{
  chrome.tabs.query(
    { url: "*://*.aeries.net/student/Dashboard*" },
    function (tabs) {
      console.log("tabs:");
      console.log(tabs);
      if (tabs.length != 0) {
...
function shareInterface() {
  chrome.tabs.query(
    { url: "*://*.aeries.net/student/Classes*" },
    function (tabs) {
      for (tab of tabs) {
        chrome.tabs.sendMessage(
          tab.id,
          { message: "share schedule" },
          function (response) {
            console.log(response);
            chrome.tabs.create({
              url: "html/share.html",
            ...
            setTimeout(function () {
              chrome.tabs.query(
                { title: "Simplify Aeries Share Schedule" },
                function (tabs) {
                  if (tabs.length > 1) {
...
function setVersion() {
  ...
  document.getElementById(
    "toggleExtention"
  ).parentElement.innerHTML = `<label><input type="checkbox" id="toggleExtention" style="text-align: center; margin-left: 0%">Use Customizations? v${manifestData.version}</label>`;
  restore_options();
  registerActions();
}
...
function registerActions() {
  document
    .getElementById("toggleExtention")
