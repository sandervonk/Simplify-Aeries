...
let timeoutID = window.setTimeout(function () {
  try {
    createPage(JSON.parse(localStorage["simplify-message"]));
  } catch {
...
function setColors([color1, color2], [color3, color4]) {
  ...
  color4 = color2;
  //}
  document.getElementById("color1").style.background = decrease_brightness(
    color1,
    30
  );
  ...
  document.getElementById(
    "bg-gradient"
  ).style.background = `linear-gradient(135deg, ${color3}, ${color4})`;
  document.getElementById("color2").style.background = decrease_brightness(
    color2,
    30
  );
}
...
window.addEventListener("load", function () {
  $("#download").click(function () {
    ...
    html2canvas(document.getElementsByClassName("display")[0]).then(function (
      canvas
    ) {
  ...
  document.querySelector(".title-input").addEventListener("input", function () {
    ...
    if (text.length === 0) {
      ...
      classElement.style.height = "80%";
    } else {
      title.style.display = "";
      classElement.style.height = "";
    }
    if (text.length > 20) {
      classElement.style.height = "70%";
    }
  });
